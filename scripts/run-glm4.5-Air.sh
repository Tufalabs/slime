...
source "${SCRIPT_DIR}/models/glm4.5-106B-A12B.sh"

CKPT_ARGS=(
   --hf-checkpoint $BASE_DIR/GLM-4.5-Air
   --ref-load $BASE_DIR/GLM-4.5-Air_dist/
   --save /mnt/artifacts/GLM-4.5-Air_dist/
   --save-interval 1
   --no-save-optim
)
...
WANDB_ARGS=(
   # --use-wandb
   # --wandb-project glm4.5-air
   # --wandb-group glm4.5-test-run-air
   # --wandb-key ${WANDB_KEY}
)
...
ray job submit --address="http://${MLP_WORKER_0_HOST}:8265" \
   ...
   --colocate \
   --save-debug-rollout-data /mnt/artifacts/zhuzilin/github-slime/data.pt \
   ${MODEL_ARGS[@]} \
   ${CKPT_ARGS[@]} \
   ${ROLLOUT_ARGS[@]} \
   ${OPTIMIZER_ARGS[@]} \
